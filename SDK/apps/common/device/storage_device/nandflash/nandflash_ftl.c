...
// FAT BPB field offsets (from virfat_flash.h)
...
// 已知限制：
// - 擦写次数仅保存在 RAM 的 ftl_hdl.block_info.block_erase_num[]，未回写 flash
// - 坏块 > 70% 阈值检查已实现（ftl_errblock_num_check），但坏块表不持久化
// - 磨损均衡使用随机选块策略，不做最小擦写次数优先（见 ftl_block_info_exchange）
// - 供电丢失无事务保护，异常断电可能导致数据损坏
// - 多个擦除块处理（ftl_block_info_exchange）、搜索优化等边缘路径留 TODO
...
static void ftl_inc_erase_count(void)
{
    u32 cnt = (u8)ftl_hdl.block_info.block_erase_num[0]
            | ((u8)ftl_hdl.block_info.block_erase_num[1] << 8)
            | ((u8)ftl_hdl.block_info.block_erase_num[2] << 16);
    cnt++;
    ftl_hdl.block_info.block_erase_num[0] = cnt & 0xFF;
    ftl_hdl.block_info.block_erase_num[1] = (cnt >> 8) & 0xFF;
...
static void ftl_block_info_exchange(void)
{
    ...
    if (0 == ftl_check_ff(buf, ftl_hdl.spbk.spbk_page_size)) {
        ...
        memset(buf, 0xff, ftl_hdl.spbk.spbk_page_size);
        //每次 都更新 loginfo 的保持一致
        /* res = __ftl_read(ftl_hdl.hdev, buf, b, page_sec); */
        /* if (res) { */
        /*     goto __exit; */
        /* } */
        /* if (ftl_check_ff(buf, ftl_hdl.spbk.spbk_page_size)) { */
        /*     goto __exit; */
        /* } */
        for (int i = 0; i < c; i += page_sec) {
            if ((i % block_sec) == 0) {
...
static int _ftl_get_plane_block(int org, u8 ex_flag)
{
    ...
    for (int i = rand_block - 2; i > ftl_hdl.spbk.spbk_cold_part_exaddr; i -= 2) {
        ...
        if (res) {
            /* ASSERT(); //读block_info失败 */
            log_error("cold_part exhausted: %d", ftl_hdl.spbk.spbk_cold_part_exaddr);
            return -1;
        }
        /* put_buf(&tmp_dir, sizeof(BLOCK_INFO)); */
        if ((tmp_dir.logic_map_addr == FTL_EX_FREE) && ((u8)tmp_dir.page_status == FTL_FREE)) {
...
int _ftl_fcopy_block(int org, int dst, int addr, u8 *wbuf, int buflen)
{
    ...
    if (addr > -1) {
        int page_num = addr / ftl_hdl.spbk.spbk_page_size;
        int offset = addr % ftl_hdl.spbk.spbk_page_size;
...
__eraser_dst:
    ...
    if (res) {
        res = FTL_DISK_ERR;
        log_error("block copy err: %d", res);
        goto __exit;
    }
__exit:
    return res;
...
static int ftl_update_spbk_info()
{
    ...
    log_info("spbk: %dK page, %dK block, flash=%d blocks, mount=%d blocks",
             ftl_hdl.spbk.spbk_page_size / 1024, ftl_hdl.spbk.spbk_block_size / 1024,
             ftl_hdl.spbk.spbk_flash_size, ftl_hdl.spbk.spbk_mount_size);
...
{
    int res = FTL_OK;
    int total_blocks = ftl_hdl.spbk.spbk_flash_size / ftl_hdl.spbk.spbk_block_size;
    if (ftl_hdl.err_block_num > total_blocks * 70 / 100) {
        res = FTL_FLASH_ERR;
    }
    return res;
}
...
static int ftl_erase_all(int cap, int block_num)
{
    ...
    int logic_cnt = 0;
    log_info("erase_all: %d clean blocks from #%d", block_num_cnt, block_i);
...
static int ftl_create_part_info(int cap, int block_num)
{
    ...
    log_info("part preset: hot=%d cold=%d coldEx=%d hotEx=%d infoLog=%d info=%d infoLen=%d",
             ftl_hdl.spbk.spbk_hot_part_addr, ftl_hdl.spbk.spbk_cold_part_addr,
             ftl_hdl.spbk.spbk_cold_part_exaddr, ftl_hdl.spbk.spbk_hot_part_exaddr,
             ftl_hdl.spbk.spbk_block_info_logaddr, ftl_hdl.spbk.spbk_block_info_addr,
             ftl_hdl.spbk.spbk_block_info_len);
...
static int ftl_format(int clust_size)
{
    ...
    log_info("fat_sector=%d, database=%d sectors", fat_sector, database);
    ...
    log_info("FAT table & DIR entry: %d - %d", fatbase + 1 + offset, database + offset);
    memset(ftl_buf, 0, 512);
    ...
    /*     goto __exit; */
    /* } */
    page_buf = zalloc(ftl_hdl.spbk.spbk_page_size);//使用最大page的长度读写（2048）
    if (fatbase * 512 % ftl_hdl.spbk.spbk_page_size) {
        memcpy(page_buf + fatbase * 512 % ftl_hdl.spbk.spbk_page_size, ftl_buf, 512);
    } else {
        memcpy(page_buf, ftl_buf, 512);
    }
    //清0 fat表和目录项 避免文件系统检查到0xff然后判断出错
    int offset = ftl_hdl.cur_block * ftl_hdl.spbk.spbk_block_size / 512;
    log_info("FAT table & DIR entry: %d - %d", fatbase + offset, database + offset);
    int first_page = (fatbase + offset) * 512 / ftl_hdl.spbk.spbk_page_size;
    int end_page = ((database + offset) * 512 + ftl_hdl.spbk.spbk_page_size - 1) / ftl_hdl.spbk.spbk_page_size;
    int first_block = ftl_hdl.cur_block;
    int all_block = ((end_page - first_page) * ftl_hdl.spbk.spbk_page_size + ftl_hdl.spbk.spbk_block_size - 1) / ftl_hdl.spbk.spbk_block_size;
    ...
    if (fatbase * 512 < ftl_hdl.spbk.spbk_page_size) {
        ...
        res = __ftl_write(ftl_hdl.hdev, page_buf, first_page * ftl_hdl.spbk.spbk_page_size / 512, ftl_hdl.spbk.spbk_page_size / 512);
        if (res) {
            res = FTL_DISK_ERR;
            goto __exit;
        }
    ...
    memset(page_buf, 0, ftl_hdl.spbk.spbk_page_size);
    int block_page = ftl_hdl.spbk.spbk_block_size / ftl_hdl.spbk.spbk_page_size;
    ...
    while (all_page > 0) {
        ...
        res = __ftl_write(ftl_hdl.hdev, page_buf, page_num * ftl_hdl.spbk.spbk_page_size / 512, ftl_hdl.spbk.spbk_page_size / 512);
        if (res) {
            res = FTL_DISK_ERR;
            goto __exit;
        }
        res = __ftl_read(ftl_hdl.hdev, page_buf, page_num * ftl_hdl.spbk.spbk_page_size / 512, ftl_hdl.spbk.spbk_page_size / 512);
        if (res) {
            res = FTL_DISK_ERR;
            goto __exit;
        }
        for (int i = 0; i < ftl_hdl.spbk.spbk_page_size; i++) {
            if (page_buf[i] != 0) {
                res = FTL_DISK_ERR;
                goto __exit;
            }
...
int _ftl_init(void)
{
    int res;
    int cap;
    ftl_hdl.spbk.spbk_page_size = 2048; // fallback for drivers without IOCTL_GET_PAGE_SIZE
    int page_size = 0;
    dev_ioctl(ftl_hdl.hdev, IOCTL_GET_PAGE_SIZE, (u32)&page_size);
    if (page_size) {
        ftl_hdl.spbk.spbk_page_size = page_size;
    }
    ftl_hdl.spbk.spbk_block_size = ftl_hdl.spbk.spbk_page_size * 64;
    res = dev_ioctl(ftl_hdl.hdev, IOCTL_GET_CAPACITY, (u32)&cap);
    if (res) {
        return FTL_DISK_ERR;
    }
    int block_num = cap / ftl_hdl.spbk.spbk_block_size; //总block数
    log_info("flash: %dKB, %d blocks", cap / 1024, block_num);
...
int _ftl_fcopy_block_bak(int org, int dst, u8 *tmp_buf, int sec_addr, u8 *wbuf, int buflen)
{
    int res;
    int block_sec = ftl_hdl.spbk.spbk_block_size / 512; // 一个block占的扇区。
    int block_page = ftl_hdl.spbk.spbk_block_size / ftl_hdl.spbk.spbk_page_size; // 一个block占的page数。
    int page_sec = ftl_hdl.spbk.spbk_page_size / 512;
    res = __ftl_erase(dst);
    ftl_inc_erase_count();
    if (res) {
...
int _ftl_write(void *buf, u32 logic_sec_addr, u32 len)
{
    int res;
    /* int rand_block; */
    /* char *tmp_buf = zalloc(ftl_hdl.spbk.spbk_page_size); //使用page大小代替512减少读写次数加快速度。 */
    int block_sec = ftl_hdl.spbk.spbk_block_size / 512; // 一个block占的扇区。
    res  = ftl_get_block_info(logic_sec_addr / block_sec, &ftl_hdl.block_info, 0);
    if (res) {
        log_error("block_info lookup failed: location=%d", logic_sec_addr / block_sec);
        goto __exit;
    }
    ...
    log_info("cur_block=%d, new_cold_sblock=%d", ftl_hdl.cur_block, ftl_hdl.new_cold_sblock);
...
int ftl_init(void)
{
    ...
    if (ftl_init_flag) {
        ...
        if (0 == memcmp(fbuf, "spbk", 4)) {
            ...
            log_info("first mount, initializing...");
            put_buf(fbuf, 512);
            res = _ftl_init();
        }
...
static int ftl_dev_init(const struct dev_node *node, void *arg)
{
    ...
    ftl_block_info_exchange();
    log_info("init: %dms", sys_timer_get_ms() - init_timer);
...
static int ftl_dev_write(struct device *device, void *buf, u32 len, u32 addr)
{
    ...
    if (len > ftl_hdl.spbk.spbk_block_size / 512) {
        return -1;
    }
