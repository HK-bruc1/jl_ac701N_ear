...
#pragma bss_seg(".icm_42670p.data.bss")
#pragma data_seg(".icm_42670p.data")
#pragma const_seg(".icm_42670p.text.const")
#pragma code_seg(".icm_42670p.text")
#endif
...
//iic or spi init
    ...
    case UI_SPI4: {
        ...
        spi_cs_init();
        spi_init(get_hw_spi_config(icm42670p_info->spi_hdl));
        /* uint8_t dummy = 0; */
...
//解码后数据: 8(timer[u64])+12(acc[float])+12(gyro[float])+4(temp[float])
//原始数据  : 8(timer[u64])+12(acc[int32])+12(gyro[int32])+2(temp[int16])
        inv_imu_get_data_from_fifo(&icm_driver);//包个数或状态
#if ICM42670P_FIFO_DATA_FIT
