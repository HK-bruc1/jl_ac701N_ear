...
extern tuya_ble_status_t tuya_ble_ota_response(tuya_ble_ota_response_t *p_data);
extern void tuya_post_key_event(u8 type);
extern int tws_ota_data_send_pend(void);
extern void db_update_notify_fail_to_phone();
extern int tws_ota_trans_to_sibling(u8 *data, u16 len);
...
extern u16 tws_ota_enter_verify_without_crc_dont_wait(void *priv);
extern u16 tws_ota_wait_verify_without_crc(void *priv);
int tuya_ota_file_end_response(void *priv)
{
    u8 res, up_flg;
    tuya_ble_ota_response_t response;
    tuya_ota_end_response_t ota_end_response;
    ota_end_response.type = 0;
    ...
    u16 ret = tws_ota_enter_verify_without_crc_dont_wait(NULL);
    if (ret) {
...
#if (OTA_TWS_SAME_TIME_ENABLE)
        if (tws_ota_wait_verify_without_crc(NULL)) {
...
void tuya_ota_proc(u16 type, u8 *recv_data, u32 recv_len)
{
    ...
    case TUYA_BLE_OTA_REQ:
        ...
        ota_req_response.type = 0;//0-ble, 1-mcu
        ota_req_response.version = tal_common_info.firmware_version;
        ota_req_response.version = READ_BIG_U32(&ota_req_response.version);
        ota_req_response.max_pkt_len = OTA_MAX_DATA_LEN;
        ota_req_response.max_pkt_len = READ_BIG_U16(&ota_req_response.max_pkt_len);
        first_packet_come = 0;
        current_file_size = 0;
    ...
    case TUYA_BLE_OTA_FILE_INFO:
        ...
        if (dual_bank_update_allow_check(tuya_ota.file_size) != 0) {
        ...
        } else if (file_version <= tal_common_info.firmware_version) {
#if (OTA_TWS_SAME_TIME_ENABLE)
            tws_ota_close();
#endif
            dual_bank_passive_update_exit(NULL);
            ...
            ota_file_info_response.store_file_len = 0;
            ota_file_info_response.store_crc = 0;
            memset(ota_file_info_response.md5, 0, sizeof(ota_file_info_response.md5));
        }
